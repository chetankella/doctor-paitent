...
import { Lock, User, Eye, EyeOff, Stethoscope, AlertCircle, CheckCircle } from 'lucide-react';
...
export default function DoctorSetupPage() {
  ...
  useEffect(() => {
    ...
    doctorAPI.verifyToken(token).then(res => {
      if (res.success) setTokenData(res.data);
      else setTokenError(res.error || 'This setup link is invalid or has already been used.');
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    const res = await doctorAPI.setPassword(token, { name: form.name, password: form.password });
    if (res.success) {
      toast.success('Doctor account activated! Redirecting to login...');
    ...
    <div className="setup-page">
      <motion.div className="setup-card" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <div className="setup-header">
          <div className="setup-logo"><Stethoscope size={22} /></div>
          <h1>Doctor Account Setup</h1>
              ...
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
                <div className="input-group">
                  ...
                  <div style={{ position: 'relative' }}>
                    ...
                    <input className="input-field" placeholder="Dr. John Smith" value={form.name} onChange={e => set('name', e.target.value)} style={{ paddingLeft: 36 }} required autoFocus />
                  </div>
                </div>
                ...
                <div style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border-light)', borderRadius: 'var(--radius-md)', padding: 'var(--space-3)', fontSize: 'var(--font-size-xs)', color: 'var(--text-secondary)' }}>
                  After setting your password, you can complete your profile (license, specialization) from your doctor dashboard.
                </div>

                <button className="btn btn-primary btn-lg" type="submit" disabled={loading} style={{ width: '100%' }}>
                  {loading ? <Spinner size={18} /> : 'Activate Doctor Account'}
                </button>
              </form>
            </>
          )}
